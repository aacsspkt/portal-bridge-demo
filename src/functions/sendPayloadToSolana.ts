...
export async function init() {
  ...
  const [config_acc, _token_bump] = await PublicKey.findProgramAddress([
    Buffer.from("config")
  ],
    program.programId);
...
export async function registerEthAddress() {
  ...
  const emmiter_acc = findProgramAddressSync([
    ...
    b.serializeUint16(CHAIN_ID_BSC)],
    program.programId
  )[0]
...
export async function send_message() {
  ...
  const vaaBytes = (
    await (
      await fetch(
        `${WORMHOLE_RPC_HOSTS}/v1/signed_vaa/${CHAIN_ID_SOLANA}/${emitterAddress}/${seq}`
      )
    ).json()
  );
...
let processed_vaa_key = findProgramAddressSync([
  ...
  let core_bridge_vaa_key = findProgramAddressSync([
    ...
    hash
  ], new anchor.web3.PublicKey(SOL_BRIDGE_ADDRESS))[0]
